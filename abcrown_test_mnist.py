#########################################################################
##   This code is based on the α,β-CROWN (alpha-beta-CROWN) verifier   ##
##                                                                     ##
##                                                                     ##
...
from bab import general_bab, general_bab_self
...
class ABCROWN_test:
        ...
        for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
            ...
            if enable_incomplete and not input_split:
                if len(init_global_lb) > 1:  # if batch == 1, there is no need to filter here.
                    ...
                    if init_verified_cond[0]:
                        init_verified_cond[0] = False
    ...
    def main(self):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            # Reset the specification to each label only
            temp_l = list()
            ub_flag = False
            for i in range(vnnlib[0][1][0][0].shape[1]):
                temp = torch.zeros(1, 10)
                temp[0][i] = 1
                temp_l.append((temp, np.array([50])))
            temp_bound = scipy.io.loadmat('self_doc/data/self_vnnlib_110.mat')
            # temp_bound = scipy.io.loadmat('self_doc/data/self_cifar_vnnlib.mat')
            vnnlib[0] = (temp_bound['array'], temp_l)

            # Reverse the specification to compute the upper bound
            ub_flag = True
            temp_l = list()
            for i in range(len(vnnlib[0][1])):
                temp1 = (-vnnlib[0][1][i][0], vnnlib[0][1][i][1])
                temp_l.append(temp1)
            vnnlib[0] = (vnnlib[0][0], temp_l)
            ...
            if (not verified_success
                ...
                verified_status, lb_list = self.complete_verifier(
            ...
            if ub_flag:
                ub_list = - (np.array(lb_list) + 50)
                print(ub_list)
            else:
                print(np.array(lb_list) + 50)
            if (bab_args['cut']['enabled'] and bab_args['cut']['cplex_cuts']
...
if __name__ == '__main__':
    abcrown = ABCROWN_test(args=sys.argv[1:])
    abcrown.main()
