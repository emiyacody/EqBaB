#########################################################################
##         This code is based on the alpha-beta-CROWN verifier         ##
##                                                                     ##
...
class BasicBlock_self(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        super(BasicBlock_self, self).__init__()
...
def ResNet18_self():
    return ResNet_self(BasicBlock_self, [2, 2, 2, 2])


class BasicBlock_cat(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, bn=False, kernel=3):
        super(BasicBlock_cat, self).__init__()
        self.bn = bn
        if kernel == 3:
            # can only do planes 16, block1
            self.conv1_1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=(not self.bn))
            self.conv1_2 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=(not self.bn))
            if self.bn:
                self.bn1 = nn.BatchNorm2d(planes)
            self.conv2_1 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=(not self.bn))
            self.conv2_2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=(not self.bn))
        else:
            exit("kernel not supported!")
    ...
    def forward(self, x):
        ...
        if self.bn:
            ...
            out = torch.cat((self.conv1_1(x[:, :, :, :W]), self.conv1_2(x[:, :, :, W:])), 3)
            out = F.relu(out)
            # print("residual relu:", out.shape, out[0].view(-1).shape)
            new_W = out.shape[3]//2
            out = torch.cat((self.conv2_1(out[:, :, :, :new_W]), self.conv2_2(out[:, :, :, new_W:])), 3)
        # out1 = self.shortcut1(x[:, :, :, W:])
        # out2 = self.shortcut2(x[:, :, :, :W])
        out += torch.cat((self.shortcut1(x[:, :, :, W:]), self.shortcut2(x[:, :, :, :W])), 3)
        out = F.relu(out)
        # print("residual relu:", out.shape, out[0].view(-1).shape)
        return out


class CResNet5_cat(nn.Module):
    def __init__(self, block, num_blocks=2, num_classes=10, in_planes=64, bn=True, last_layer="avg"):
        super(CResNet5_cat, self).__init__()
        self.in_planes = in_planes
        self.bn = bn
        self.last_layer = last_layer
        self.conv1_1 = nn.Conv2d(3, in_planes, kernel_size=3,
                               stride=2, padding=1, bias=not self.bn)
        self.conv1_2 = nn.Conv2d(3, in_planes, kernel_size=3,
                                 stride=2, padding=1, bias=not self.bn)
        if self.bn:
            self.bn1 = nn.BatchNorm2d(in_planes)
        self.layer1 = self._make_layer(block, in_planes*2, num_blocks, stride=2, bn=bn, kernel=3)
        # self.layer1_2 = self._make_layer(block, in_planes*2, num_blocks, stride=2, bn=bn, kernel=3)
        if self.last_layer == "avg":
            self.avg2d = nn.AvgPool2d(4)
            self.linear = nn.Linear(in_planes * 8 * block.expansion, num_classes)
        elif self.last_layer == "dense":
            self.linear1_1 = nn.Linear(in_planes * 8 * block.expansion * 16, 100)
            self.linear1_2 = nn.Linear(in_planes * 8 * block.expansion * 16, 100)
            self.linear2_1 = nn.Linear(100, num_classes)
            self.linear2_2 = nn.Linear(100, num_classes)
        else:
            exit("last_layer type not supported!")
    ...
    def forward(self, x):
        if self.bn:
            ...
            out2 = F.relu(self.conv1_2(x))
        out = torch.cat((out1, out2), 3)
        # print("conv1 relu", out.shape, out[0].view(-1).shape)
        out = self.layer1(out)
        # out2 = self.layer1_2(out[:, :, :, W:])
        # out = torch.cat((out1, out2), 3)
        # print("layer1", out.shape)
        if self.last_layer == "avg":
            out = self.avg2d(out)
            ...
            # out1 = out[:, :, :, :W]
            # out2 = out[:, :, :, W:]
            out1 = out[:, :, :, :W].reshape(out1.size(0), -1)
            out2 = out[:, :, :, W:].reshape(out2.size(0), -1)
            # out = out.view(out.size(0), -1)
            # print("view", out.shape)
            # W = out.shape[1]//2
            # out1 = self.linear1_1(out[:, :W])
            # out2 = self.linear1_2(out[:, W:])
            out1 = self.linear1_1(out1)
            out2 = self.linear1_2(out2)
            out = F.relu(torch.cat((out1, out2), 1))
            # print("linear1 relu", out.shape, out[0].view(-1).shape)
            W = out.shape[1]//2
            out1 = self.linear2_1(out[:, :W])
            out2 = self.linear2_2(out[:, W:])
            print(out1)
            out = out1 - out2
            # print("output", out.shape)
        return out
...
def resnet2b_self():
    return CResNet5_cat(BasicBlock_cat, num_blocks=2, in_planes=8, bn=False, last_layer="dense")


class BasicBlock_cat_c(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, bn=False, kernel=3):
        super(BasicBlock_cat_c, self).__init__()
        self.bn = bn
        if kernel == 3:
            # can only do planes 16, block1
            self.conv1_1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=(not self.bn))
    ...
    def forward(self, x):
        ...
        if self.bn:
            ...
            out = torch.cat((self.conv1_1(x[:, :C, :, :]), self.conv1_2(x[:, C:, :, :])), 1)
            out = F.relu(out)
            # print("residual relu:", out.shape, out[0].view(-1).shape)
            new_C = out.shape[1]//2
            out = torch.cat((self.conv2_1(out[:, :new_C, :, :]), self.conv2_2(out[:, new_C:, :, :])), 1)
        # out1 = self.shortcut1(x[:, :, :, W:])
        # out2 = self.shortcut2(x[:, :, :, :W])
        out += torch.cat((self.shortcut1(x[:, :C, :, :]), self.shortcut2(x[:, C:, :, :])), 1)
        out = F.relu(out)
        # print("residual relu:", out.shape, out[0].view(-1).shape)
        return out


class CResNet5_cat_c(nn.Module):
    def __init__(self, block, num_blocks=2, num_classes=10, in_planes=64, bn=True, last_layer="avg"):
        super(CResNet5_cat_c, self).__init__()
        self.in_planes = in_planes
        self.bn = bn
        self.last_layer = last_layer
        self.conv1_1 = nn.Conv2d(3, in_planes, kernel_size=3,
    ...
    def forward(self, x):
        ...
        if self.last_layer == "avg":
            ...
            # out1 = out[:, :, :, :W]
            # out2 = out[:, :, :, W:]
            out1 = out[:, :C, :, :].reshape(out1.size(0), -1)
            out2 = out[:, C:, :, :].reshape(out2.size(0), -1)
            # out = out.view(out.size(0), -1)
            # print("view", out.shape)
            # W = out.shape[1]//2
            # out1 = self.linear1_1(out[:, :W])
...
def resnet2b_c_self():
    return CResNet5_cat_c(BasicBlock_cat_c, num_blocks=2, in_planes=8, bn=False, last_layer="dense")


class MNIST_FNN_net_cat(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear1 = nn.Linear(28 * 28 * 2, 512 * 2)
        self.linear2 = nn.Linear(512 * 2, 128 * 2)
        self.linear3 = nn.Linear(128 * 2, 32 * 2)
        self.linear4 = nn.Linear(32 * 2, 10 * 2)
...
class MNIST_CNN_net_cat(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1_1 = nn.Conv2d(1, 8, kernel_size=3, stride=2, padding=1)
        self.conv1_2 = nn.Conv2d(1, 8, kernel_size=3, stride=2, padding=1)
        self.conv2_1 = nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=1)
        self.conv2_2 = nn.Conv2d(8, 16, kernel_size=3, stride=2, padding=1)
        self.fc1 = nn.Linear(7 * 7 * 16 * 2, 128 * 2)
        self.fc2 = nn.Linear(128 * 2, 10 * 2)
    def forward(self, x):
...
class VGG_net_cat(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1_1_1 = nn.Conv2d(3, 32, 3, padding=1)
        self.conv1_2_1 = nn.Conv2d(32, 32, 3, padding=1, stride=2)
        self.conv2_1_1 = nn.Conv2d(32, 64, 3, padding=1)
        self.conv2_2_1 = nn.Conv2d(64, 64, 3, padding=1, stride=2)
        self.conv3_1_1 = nn.Conv2d(64, 128, 3, padding=1)
        self.conv3_2_1 = nn.Conv2d(128, 128, 3, padding=1)
        self.conv3_3_1 = nn.Conv2d(128, 128, 3, padding=1, stride=2)
        self.conv4_1_1 = nn.Conv2d(128, 256, 3, padding=1)
        self.conv4_2_1 = nn.Conv2d(256, 256, 3, padding=1)
        self.conv4_3_1 = nn.Conv2d(256, 256, 3, padding=1, stride=2)
        self.conv5_1_1 = nn.Conv2d(256, 512, 3, padding=1)
        self.conv5_2_1 = nn.Conv2d(512, 512, 3, padding=1)
        self.conv5_3_1 = nn.Conv2d(512, 512, 3, padding=1, stride=2)
        self.conv1_1_2 = nn.Conv2d(3, 32, 3, padding=1)
        self.conv1_2_2 = nn.Conv2d(32, 32, 3, padding=1, stride=2)
        self.conv2_1_2 = nn.Conv2d(32, 64, 3, padding=1)
        self.conv2_2_2 = nn.Conv2d(64, 64, 3, padding=1, stride=2)
        self.conv3_1_2 = nn.Conv2d(64, 128, 3, padding=1)
        self.conv3_2_2 = nn.Conv2d(128, 128, 3, padding=1)
        self.conv3_3_2 = nn.Conv2d(128, 128, 3, padding=1, stride=2)
        self.conv4_1_2 = nn.Conv2d(128, 256, 3, padding=1)
        self.conv4_2_2 = nn.Conv2d(256, 256, 3, padding=1)
        self.conv4_3_2 = nn.Conv2d(256, 256, 3, padding=1, stride=2)
        self.conv5_1_2 = nn.Conv2d(256, 512, 3, padding=1)
        self.conv5_2_2 = nn.Conv2d(512, 512, 3, padding=1)
        self.conv5_3_2 = nn.Conv2d(512, 512, 3, padding=1, stride=2)
        self.fc1 = nn.Linear(512 * 2, 128 * 2)
        self.fc2 = nn.Linear(128 * 2, 32 * 2)
        self.fc3 = nn.Linear(32 * 2, 10 * 2)
    def forward(self, x):
        x1 = F.relu(self.conv1_2_1(self.conv1_1_1(x)))
        x2 = F.relu(self.conv1_2_2(self.conv1_1_2(x)))
        x1 = F.relu(self.conv2_2_1(self.conv2_1_1(x1)))
        x2 = F.relu(self.conv2_2_2(self.conv2_1_2(x2)))
        x1 = F.relu(self.conv3_3_1(self.conv3_2_1(self.conv3_1_1(x1))))
        x2 = F.relu(self.conv3_3_2(self.conv3_2_2(self.conv3_1_2(x2))))
        x1 = F.relu(self.conv4_3_1(self.conv4_2_1(self.conv4_1_1(x1))))
        x2 = F.relu(self.conv4_3_2(self.conv4_2_2(self.conv4_1_2(x2))))
        x1 = F.relu(self.conv5_3_1(self.conv5_2_1(self.conv5_1_1(x1))))
        x2 = F.relu(self.conv5_3_2(self.conv5_2_2(self.conv5_1_2(x2))))
        x1 = x1.reshape(x.size(0), -1)
        x2 = x2.reshape(x.size(0), -1)
        x = torch.cat((x1, x2), 1)
        x = F.relu(self.fc1(x))
